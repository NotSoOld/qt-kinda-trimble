...
QString PacketParser::analyseAndParse()
{
    ...
    case REPORT_SUPER:
        ...
        break;
    default:
        // Если код пакета никуда не подошел...
...
// Этот пакет присылается GPS-модулем, если тот не смог разобрать, что до этого отправили мы.
...
// и если соответствующие настройки задают присылать его.
...
// Приходит вместе с запросом позиции и скорости (и прочей информации, которую пользователь указал в настройках присылать).
...
// Не отображается в интерфейсе по причине гигантских размеров, полностью высылается в лог.
...
// не очень правильно работают, но это опасно - согласно документации).
...
QString PacketParser::parse_REPORT_STATUS_SATELLITE_HEALTH()
{
    ...
    if ((int)data[0] == 3) {
        ...
        for (int i = 1; i <= 32; i++) {
            info.append(QString("№%0 - %1  \t").arg(i).arg((quint8)data[i] == (quint8)0 ? "включ." : "выключ."));
            if (i % 4 == 0)
                info.append("\n");
        } 
...
// Не отображается в интерфейсе. Содержатся служебные параметры низкого уровня об измерениях спутника
...
// Всё остальное высылается в лог.
...
// Не отображается в интерфейсе, т.к. этот пакет дублирует то, что отправляет пользователь.
