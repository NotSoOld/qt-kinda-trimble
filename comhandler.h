...
public:
    ...
    // Раньше этот класс действительно должен был быть многопоточным. В процессе разработки в этом отпала необходимость,
    // т.к. прием и отправка пакетов были переведены на срабатывание по сигналам, и конфликты разрешились сами собой.
    // TODO: удалить эти архаизмы и поменять класс-родитель
    void run();
    void (COMHandler::*methodToStartThreadWith)();

    // Статические члены класса: имя порта, сам объект порта VirtualCOM, массив текущей прочитанной информации из порта
