...
{
    bool ecefChecked = QMLDataObtainer::getBoolFromQML(window, "eCEFcheck", "checked");
    bool llaChecked = QMLDataObtainer::getBoolFromQML(window, "lLAcheck", "checked");
    bool precision = QMLDataObtainer::getBoolFromQML(window, "doublePrecRB", "checked");
    bool gpsTime = QMLDataObtainer::getBoolFromQML(window, "gpsTimeRB", "checked");
    bool mslChecked = QMLDataObtainer::getBoolFromQML(window, "mslRB", "checked");
    bool ecefVelChecked = QMLDataObtainer::getBoolFromQML(window, "ecef_vel_check", "checked");
    bool enuVelChecked = QMLDataObtainer::getBoolFromQML(window, "enu_vel_check", "checked");
    bool raw_data_report_checked = QMLDataObtainer::getBoolFromQML(window, "raw_data_report_check", "checked");
    bool dbhz_out_checked = QMLDataObtainer::getBoolFromQML(window, "dbhz_out_RB", "checked");

    QByteHelper::appendAndStuff(cmd, (byte)(
               (ecefChecked ? BIT(0) : 0) |
               (llaChecked ? BIT(1) : 0) |
               (mslChecked ? BIT(2) : 0) |
               (precision ? BIT(4) : 0)
    ...
    QByteHelper::appendAndStuff(cmd, (byte)(gpsTime ? 0 : BIT(0)));
    QByteHelper::appendAndStuff(cmd, (byte)(
...
{
    double initX = QMLDataObtainer::getDoubleFromQML(window, "init_x_text", "text");
    double initY = QMLDataObtainer::getDoubleFromQML(window, "init_y_text", "text");
    double initZ = QMLDataObtainer::getDoubleFromQML(window, "init_z_text", "text");
    QByteHelper::appendAndStuff(cmd, initX);
    QByteHelper::appendAndStuff(cmd, initY);
    QByteHelper::appendAndStuff(cmd, initZ);
...
{
    double initLat = QMLDataObtainer::getDoubleFromQML(window, "init_lat_text", "text");
    double initLong = QMLDataObtainer::getDoubleFromQML(window, "init_long_text", "text");
    double initAlt = QMLDataObtainer::getDoubleFromQML(window, "init_alt_text", "text");
    QByteHelper::appendAndStuff(cmd, initLat);
    QByteHelper::appendAndStuff(cmd, initLong);
    QByteHelper::appendAndStuff(cmd, initAlt);
...
{
    byte typeOfData = (byte)QMLDataObtainer::getIntFromQML(window, "typeOfDataComboBox", "currentIndex") + 2;
    byte satelliteIndex = (byte)QMLDataObtainer::getIntFromQML(window, "satellite_selection_spinner1", "value");
    QByteHelper::appendAndStuff(cmd, typeOfData);
    QByteHelper::appendAndStuff(cmd, satelliteIndex);
...
{
    byte satelliteIndex = (byte)QMLDataObtainer::getIntFromQML(window, "satellites_and_health_spinner", "value");
    QByteHelper::appendAndStuff(cmd, satelliteIndex);
...
{
    bool maskPrimaryPackets = QMLDataObtainer::getBoolFromQML(window, "primaryPacketMaskingBit", "checked");
    bool maskSupplPackets = QMLDataObtainer::getBoolFromQML(window, "supplPacketMaskingBit", "checked");
    bool maskOtherPackets = QMLDataObtainer::getBoolFromQML(window, "otherPacketsMaskingBit", "checked");
    QByteHelper::appendAndStuff(cmd, (byte)(
                       (maskPrimaryPackets ? BIT(0) : 0) |
                       (maskSupplPackets ? BIT(2) : 0) |
                       (maskOtherPackets ? BIT(6) : 0)
                      ));
    QByteHelper::appendAndStuff(cmd, (byte)0);
    QByteHelper::appendAndStuff(cmd, (byte)0);
    QByteHelper::appendAndStuff(cmd, (byte)0);
...
{
    byte typeOfRequest = (byte)QMLDataObtainer::getIntFromQML(window, "timingPacketItem", "timingPacketType");
    QByteHelper::appendAndStuff(cmd, typeOfRequest);
...
void COMHandler::send_command(int code, int subcode)
{
    ...
    QByteHelper::appendAndStuff(&cmd, (byte)code);
    if (subcode > 0) {
        QByteHelper::appendAndStuff(&cmd, (byte)subcode);
    }
